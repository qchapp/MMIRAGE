...
| Shard recovery | Kubernetes or Run:ai namespace, H100 access, `kubectl`, and a shared ReadWriteMany PVC. A local signal-based fallback is also provided. | `python experiments/shard_recovery/scripts/run_k8s.py --help` and `python experiments/shard_recovery/scripts/run_local.py --help`; native competitor manifests use `python experiments/shard_recovery/scripts/plan_native_competitor_recovery.py --framework all --condition all`. | [`experiments/shard_recovery/README.md`](experiments/shard_recovery/README.md) |
...
| MMIRAGE vs NeMo Curator comparison | One H100-80GB for the full shared-SGLang ChartQA run, plus isolated MMIRAGE, NeMo, and planned DataTrove environments. | Follow the mock-server smoke check in the runbook; `run_comparison.py --dry-run` validates planned commands without running either framework; DataTrove planned command uses `python experiments/nemo_curator_comparison/scripts/run_comparison.py --order datatrove --repetitions 1 --dry-run`. | [`experiments/nemo_curator_comparison/README.md`](experiments/nemo_curator_comparison/README.md) |
