"""Shard status and retry helpers for the AnonLib CLI."""
...
from anonlib.cli_utils.slurm import submit_slurm_job
...
def check_failed_shards(cfg: AnonLibConfig) -> Tuple[List[int], ShardSummary]:
...
def collect_bench_stats(cfg: AnonLibConfig) -> Dict[str, Any]:
    ...
    - ``per_shard``: list of dicts, one per shard, each containing the full
      :class:`~anonlib.shard_utils.ShardStatus` payload plus a flattened
      ``stats`` sub-dict.
