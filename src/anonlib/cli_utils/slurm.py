"""SLURM helpers for the AnonLib CLI."""
...
from anonlib.cli_utils.runtime import create_directories, expand_path, get_project_root
...
def build_sbatch_script(
    ...
    shard_process_path = os.path.join(src_root, "anonlib", "shard_process.py")
    ...
    lines = [
        ...
        f"export ANONLIB_CONFIG={_bash_double_quote(config_path)}",
    ]
    if collect_stats:
        lines.append("export ANONLIB_COLLECT_STATS=1")
    sglang = get_sglang_server_config(cfg)
    if sglang is not None:
        lines.append(
            f"export ANONLIB_SHARD_IDS={_bash_double_quote(','.join(map(str, shard_ids or [])))}"
        )
    lines.extend(
        [
    ...
    python_setup = (
        ...
        '|| { echo "AnonLib requires Python >= 3.10 on compute nodes" >&2; exit 2; }; '
    )
    if sglang is not None:
        lines.append(
            'srun "${srun_args[@]}" bash -c \''
            + python_setup
            + 'exec ${PYTHON_CMD} -m anonlib.sglang_job --config "$ANONLIB_CONFIG" '
            '--shard-ids "$ANONLIB_SHARD_IDS"\''
        )
        lines.append('echo "Shared SGLang AnonLib job completed"')
    else:
        lines.append(
            'srun "${srun_args[@]}" bash -c \''
            + python_setup
            + 'exec ${PYTHON_CMD} "$SHARD_PROCESS" --config "$ANONLIB_CONFIG"\''
        )
...
def wait_for_slurm_job(job_id: int, cfg: AnonLibConfig) -> None:
...
def require_slurm(cfg: AnonLibConfig, command_name: str) -> int:
