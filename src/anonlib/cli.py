"""Command-line interface for AnonLib pipeline."""
...
from anonlib.cli_utils.runtime import setup_runtime, validate_edf_env_path
from anonlib.cli_utils.slurm import require_slurm, submit_slurm_job, wait_for_slurm_job
from anonlib.cli_utils.status import (
    check_failed_shards,
    collect_bench_stats,
    get_shard_status,
...
from anonlib.core.process.processors.image_gen.sglang_server import (
    ANONLIB_SGLANG_BASE_URL,
    get_sglang_server_config,
    shared_sglang_server,
)
from anonlib.merge_shards import MergeReport, merge_from_config, merge_input_dir
...
def run_local(
    ...
    Args:
        config_path: Absolute path to the AnonLib YAML config file.
    ...
    """
    cfg = load_anonlib_config(config_path)
    sglang = get_sglang_server_config(cfg)
    if sglang is not None and not os.environ.get(ANONLIB_SGLANG_BASE_URL):
        with shared_sglang_server(sglang):
            return run_local(config_path, shard_id, collect_stats)

    command = [sys.executable, "-m", "anonlib.shard_process", "--config", config_path]
    env = os.environ.copy()
...
def launch_pipeline(
    ...
    Args:
        cfg: Parsed AnonLib configuration object.
        config_path: Absolute path to the AnonLib YAML config file.
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    """
    parser = argparse.ArgumentParser(description="AnonLib command-line interface")
    ...
    merge_parser.add_argument(
        ...
        default=None,
        help=(
            "Optional root directory for merged outputs. AnonLib creates one "
...
def handle_run(args: argparse.Namespace, cfg: AnonLibConfig, config_path: str) -> int:
...
def handle_submit(
    args: argparse.Namespace, cfg: AnonLibConfig, config_path: str
) -> int:
...
def handle_check(args: argparse.Namespace, cfg: AnonLibConfig, config_path: str) -> int:
...
def handle_retry(args: argparse.Namespace, cfg: AnonLibConfig, config_path: str) -> int:
...
def handle_stats(
    args: argparse.Namespace, cfg: AnonLibConfig, _config_path: str
) -> int:
    ...
    Args:
        args: Parsed CLI namespace.
        cfg: Parsed AnonLib configuration object.
        _config_path: Absolute path to the AnonLib YAML config file (not needed here).
