"""Shared SGLang Diffusion server lifecycle for AnonLib orchestration."""
...
from anonlib.core.process.processors.image_gen.config import SGLangBackendConfig

logger = logging.getLogger(__name__)

ANONLIB_SGLANG_BASE_URL = "ANONLIB_SGLANG_BASE_URL"
...
def launch_sglang_server(config: SGLangBackendConfig) -> subprocess.Popen[bytes]:
    ...
    if executable is None:
        raise RuntimeError(
            ...
            "environment containing SGLang before running AnonLib."
        )
    ...
    setattr(proc, "_anonlib_output_tail", output_tail)
    threading.Thread(
...
def shared_sglang_server(config: Optional[SGLangBackendConfig]) -> Iterator[None]:
    ...
    previous_base_url = os.environ.get(ANONLIB_SGLANG_BASE_URL)
    ...
    try:
        while len(attempted_ports) < SGLANG_PORT_SEARCH_ATTEMPTS:
            ...
            try:
                ...
                os.environ[ANONLIB_SGLANG_BASE_URL] = config.resolved_base_url()
                yield
                return
                ...
                if previous_base_url is None:
                    os.environ.pop(ANONLIB_SGLANG_BASE_URL, None)
                else:
                    os.environ[ANONLIB_SGLANG_BASE_URL] = previous_base_url
...
def _format_output_tail(proc: subprocess.Popen[bytes]) -> str:
    output_tail: Sequence[str] = getattr(proc, "_anonlib_output_tail", ())
