...
from anonlib.core.process.base import BaseProcessor, ProcessorRegistry, TokenCounts
from anonlib.core.process.processors.image_gen.backends.base import (
    ImageGenerationBackend,
)
from anonlib.core.process.processors.image_gen.config import (
    ImageGenConfig,
    ImageGenOutputVar,
    ImageOutputMode,
)
...
def _create_backend(config: ImageGenConfig) -> ImageGenerationBackend:
    ...
    from anonlib.core.process.processors.image_gen.backends.sglang_backend import (
        SGLangImageBackend,
    )
    ...
    if config.backend == "external":
        ...
        base_url = os.environ.get(ANONLIB_SGLANG_BASE_URL)
        if not base_url:
            raise RuntimeError(
                "backend='sglang' requires the AnonLib runner to launch the "
                "shared SGLang server first. No ANONLIB_SGLANG_BASE_URL was provided."
            )
    else:
