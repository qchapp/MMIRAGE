"""Main script for processing dataset shards with AnonLib.
...
from anonlib.core.loader.base import DatasetLike
from anonlib.core.loader.utils import load_datasets_from_configs
from anonlib.core.process.mapper import AnonLibMapper
from anonlib.core.process.variables import OutputVar
from anonlib.core.writer.renderer import TemplateRenderer
from anonlib.shard_utils import (
...
def rewrite_batch(
    ...
    Args:
        ...
        mapper: AnonLibMapper for processing transformations.
...
def main():
    ...
    Loads configuration, datasets, processes the shard using AnonLib
    transformations (including multimodal), and saves the result to disk.
    """
    ap = argparse.ArgumentParser("Process dataset shards using AnonLib with SGLang.")
    ap.add_argument(
        "--config",
        help="YAML config for AnonLib pipeline.",
        required=True,
    )
    args = ap.parse_args()
    ...
    collect_stats = os.environ.get("ANONLIB_COLLECT_STATS", "") == "1"
    if collect_stats:
