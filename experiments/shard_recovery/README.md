...
- `anonlib_recovery.yaml`: fixed 16-shard AnonLib workload using `Qwen/Qwen3-4B`, SGLang, one tensor-parallel GPU, deterministic temperature `0.0`, and stats enabled by the pod wrapper.
- `scripts/benchmarks/prepare_shard_recovery_dataset.py`: downloads and freezes a public Hugging Face subset.
- `scripts/benchmarks/run_shard_recovery.py`: creates one Kubernetes pod per shard, terminates deterministic selected pods, and retries only shards selected from AnonLib state.
- `scripts/benchmarks/run_shard_recovery_pod.py`: pod entrypoint that runs `anonlib.shard_process` and records deliberate pod termination as shard failure in AnonLib state.
- `scripts/benchmarks/extract_shard_recovery_results.py`: writes `recovery_results.csv` and `recovery_results.json`.
...
python scripts/benchmarks/prepare_shard_recovery_dataset.py \
...
```bash
python scripts/benchmarks/run_shard_recovery.py run-condition \
  --condition baseline \
  --rep 1 \
  --namespace "$NAMESPACE" \
  --pvc "$PVC_NAME" \
...
```bash
python scripts/benchmarks/run_shard_recovery.py retry \
  --condition fail_1 \
  --rep 1 \
  --namespace "$NAMESPACE" \
  --pvc "$PVC_NAME" \
...
```bash
python scripts/benchmarks/run_shard_recovery.py run-condition --condition fail_4 --rep 1 --namespace "$NAMESPACE" --pvc "$PVC_NAME" --image "$IMAGE" --shared-root "$ANONLIB_RECOVERY_ROOT" --max-active-shards "$MAX_ACTIVE_SHARDS" --overwrite
python scripts/benchmarks/run_shard_recovery.py retry --condition fail_4 --rep 1 --namespace "$NAMESPACE" --pvc "$PVC_NAME" --image "$IMAGE" --shared-root "$ANONLIB_RECOVERY_ROOT" --max-active-shards "$MAX_ACTIVE_SHARDS"
...
```bash
python scripts/benchmarks/run_shard_recovery.py run-condition --condition fail_8 --rep 1 --namespace "$NAMESPACE" --pvc "$PVC_NAME" --image "$IMAGE" --shared-root "$ANONLIB_RECOVERY_ROOT" --max-active-shards "$MAX_ACTIVE_SHARDS" --overwrite
python scripts/benchmarks/run_shard_recovery.py retry --condition fail_8 --rep 1 --namespace "$NAMESPACE" --pvc "$PVC_NAME" --image "$IMAGE" --shared-root "$ANONLIB_RECOVERY_ROOT" --max-active-shards "$MAX_ACTIVE_SHARDS"
...
```bash
python scripts/benchmarks/run_shard_recovery.py status \
  --condition fail_4 \
  --rep 1 \
  --shared-root "$ANONLIB_RECOVERY_ROOT"
```
...
anonlib stats --config experiments/shard_recovery/anonlib_recovery.yaml
```
...
```bash
python scripts/benchmarks/extract_shard_recovery_results.py \
  --shared-root "$ANONLIB_RECOVERY_ROOT" \
...
```bash
python scripts/benchmarks/run_shard_recovery.py run-condition --condition baseline --rep 2 --namespace "$NAMESPACE" --pvc "$PVC_NAME" --image "$IMAGE" --shared-root "$ANONLIB_RECOVERY_ROOT" --max-active-shards "$MAX_ACTIVE_SHARDS" --overwrite
```
