...
1. environment and four-H100 preflight;
...
The A100 driver refuses to execute if the Git commit or workload hashes differ from the H100 manifest. It verifies that the current upstream model revision is still the exact H100 revision, downloads that exact snapshot, switches to offline mode, and runs only the four-GPU scaling transfer point plus the one-GPU endpoint-overhead experiment.
...
A publication artifact should not be packaged if the verifier reports any executable-logic, configuration, dependency, syntax, or semantic-plan difference from the frozen baseline.
