#!/usr/bin/env python3
"""Run unmodified MMIRAGE while forwarding SGLang Engine calls to an OpenAI VLM endpoint.

This benchmark-only adapter patches ``sglang.Engine`` so MMIRAGE's stock LLM
...
``src/mmirage`` package stays untouched.
"""
...
class EndpointEngine:
    ...
    def __init__(self, **kwargs: Any) -> None:
        self.model_path = kwargs.get("model_path") or os.environ["MMIRAGE_CHARTQA_MODEL_PATH"]
        self.base_url = os.environ["MMIRAGE_CHARTQA_OPENAI_BASE_URL"].rstrip("/")
        self.timeout_seconds = int(os.environ.get("MMIRAGE_CHARTQA_TIMEOUT_SECONDS", "900"))
        self.max_workers = max(1, int(os.environ.get("MMIRAGE_CHARTQA_CONCURRENCY", "64")))
        try:
