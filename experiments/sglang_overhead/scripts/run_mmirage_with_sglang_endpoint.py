...
class EndpointEngine:
    ...
    def __init__(self, **kwargs: Any) -> None:
        self.model_path = (
            kwargs.get("model_path") or os.environ["MMIRAGE_OVERHEAD_MODEL_PATH"]
        )
        self.base_url = os.environ["MMIRAGE_OVERHEAD_SGLANG_BASE_URL"].rstrip("/")
        self.timeout_seconds = int(
            os.environ.get("MMIRAGE_OVERHEAD_TIMEOUT_SECONDS", "900")
        )
        self.max_workers = max(
            1, int(os.environ.get("MMIRAGE_OVERHEAD_CONCURRENCY", "64"))
        )
