#!/usr/bin/env python3
"""Run native VLM competitor baselines for the vlm_enrichment task.

Splits the MedTrinity demo rows into one contiguous shard per visible GPU,
launches one ``experiments._shared.vlm_runners`` worker subprocess per GPU
(pinned via ``CUDA_VISIBLE_DEVICES``), then aggregates every repetition and
validates the output contract ({id, formatted_description}).
"""
...
FRAMEWORK_BACKENDS = {
    "sglang": "one SGLang server per shard worker with the shared OpenAI client",
    "datatrove": "DataTrove InferenceRunner with MediaType.IMAGE plus rollout",
    "nemo_curator": "NeMo Curator OpenAIClient pipeline with JsonlReader and JsonlWriter",
}

OUTPUT_CONTRACT = {
    "required_fields": ["id", "formatted_description"],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--framework", choices=sorted(FRAMEWORK_BACKENDS), required=True)
...
def build_manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "image_base_path": str(Path(args.image_base_path).resolve()),
        "input_rows": rows,
        "model": args.model,
...
def launch_and_wait(args: argparse.Namespace, run_dir: Path, gpu_ids: list[str]) -> dict[str, Any]:
    ...
    for shard_id, gpu_id in enumerate(gpu_ids):
        command = [
            worker_python,
            "-m",
            "experiments._shared.vlm_runners",
            "--framework",
            args.framework,
            "--input-jsonl",
...
def merge_shard_outputs(run_dir: Path, gpu_count: int) -> list[dict[str, Any]]:
    output_rows = []
...
def validate_output(input_rows: list[dict[str, Any]], output_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    input_indexes = [int(row["source_index"]) for row in input_rows]
    output_indexes = [int(row.get("source_index", -1)) for row in output_rows]
    required = OUTPUT_CONTRACT["required_fields"]

    schema_valid = 0
    schema_invalid_rows: list[str] = []
    for output_row in output_rows:
        missing = [field for field in required if output_row.get(field) in (None, "")]
        if missing:
            schema_invalid_rows.append(f"{output_row.get('id')}:missing={missing}")
        else:
            schema_valid += 1
    ...
    checks = {
        ...
        "id_set_matches_input": not missing_ids and not unexpected_ids,
...
def run_repetitions(args: argparse.Namespace, output_root: Path) -> None:
    from run_mmirage_vlm import aggregate_repetition, write_all_outputs
    ...
    for repetition in range(1, args.repetitions + 1):
        ...
        if result["failures"]:
            for failure in result["failures"]:
                print(
                    f"[{args.framework}] shard {failure['shard_id']} on GPU {failure['gpu_id']} failed with rc={failure['returncode']}: {failure['log']}",
                    file=sys.stderr,
                )
            raise SystemExit(f"{len(result['failures'])} shard worker(s) failed for {args.framework} gpu={args.gpu_count} rep={repetition}")

        merged = merge_shard_outputs(run_dir, args.gpu_count)
...
def main() -> None:
    ...
    if args.aggregate_only:
        from run_mmirage_vlm import write_all_outputs
