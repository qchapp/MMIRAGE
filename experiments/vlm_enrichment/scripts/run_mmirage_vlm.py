#!/usr/bin/env python3
"""Run and aggregate a single-node MMIRAGE 4-GPU vlm_enrichment point.

Shards the MedTrinity demo rows across ``gpu_count`` data-parallel MMIRAGE
shard workers (one per visible GPU), then aggregates the per-shard
``status.json`` statistics into the shared benchmark schema.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--vlm-recipe", default=None)
    parser.add_argument("--rows-jsonl", default=None)
...
def environment_metadata() -> Dict[str, Any]:
    ...
    return {
        ...
        "interpretation": "single-node data-parallel MMIRAGE vlm_enrichment (MedTrinity demo)",
...
def concrete_mmirage_config(
    ...
    payload = read_yaml(recipe)
    loading = payload["loading_params"]
    loading["state_dir"] = str(state_dir)
    dataset = loading["datasets"][0]
    dataset["path"] = str(rows_jsonl)
    dataset["output_dir"] = str(output_dir)
    dataset["image_base_path"] = str(image_base_path)
    loading["num_shards"] = num_shards
    loading["shard_id"] = shard_id
    loading["batch_size"] = batch_size
    payload.setdefault("execution_params", {})
    payload["execution_params"].update({"mode": "local", "retry": False, "merge": False, "max_retries": 0})
    return payload
...
def launch_shards(
    ...
    try:
        for proc, log_handle, log_path, item in processes:
            ...
            if returncode != 0:
                failures.append(
                    {"shard_id": item["shard_id"], "gpu_id": item["gpu_id"], "returncode": returncode, "log": str(log_path)}
                )
...
def build_summary(rows: List[Dict[str, Any]], metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for gpu_count in sorted(grouped):
        ...
        summary_rows.append({"gpu_count": gpu_count, "repetitions": len(group), "metrics": metrics})

    return {
        "created_at": datetime.now(timezone.utc).isoformat(),
        "interpretation": "single-node data-parallel MMIRAGE vlm_enrichment on the MedTrinity demo subset",
        "metric_definitions": {
...
def write_latex_table(path: Path, summary: Dict[str, Any]) -> None:
    ...
    lines = [
        "\\begin{tabular}{rrrr}",
        "\\toprule",
        f"GPUs & Output tok/s & Output tok/s/GPU & Rows/s {line_end}",
        "\\midrule",
    ]
    for item in summary["summary"]:
        metrics = item["metrics"]
        lines.append(
            ...
            f"{latex_cell(metrics['output_tok_s_per_gpu'])} & {latex_cell(metrics['rows_s'])} {line_end}"
        )
    lines.extend(["\\bottomrule", "\\end{tabular}"])
    lines.append("% Single-node MMIRAGE vlm_enrichment on the MedTrinity demo subset.")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    recipe = resolve_path(cfg["vlm_recipe"])
    rows_jsonl = resolve_path(cfg["rows_jsonl"])
    image_base_path = resolve_path(cfg["image_base_path"])
    if not recipe.exists():
        raise FileNotFoundError(recipe)
    if not rows_jsonl.exists() and not args.dry_run:
        raise FileNotFoundError(rows_jsonl)
    if not image_base_path.exists() and not args.dry_run:
        raise FileNotFoundError(image_base_path)

    extra_env = {str(k): str(v) for k, v in (cfg.get("environment") or {}).items()}
    workload_metadata_path = rows_jsonl.parent / "metadata.json"
