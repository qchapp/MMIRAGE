...
All paths use `Qwen/Qwen3-VL-4B-Instruct`, temperature `0.1`, top-p `0.9`, and a maximum of `1,024` new tokens. MMIRAGE uses batch size `64`; native runners are configured with concurrency `64`. The experiment uses GPUs `0`, `1`, `2`, and `3`, with three measured repetitions per framework.
