#!/usr/bin/env python3
"""Run unmodified AnonLib while forwarding sglang.Engine calls to HTTP SGLang."""
...
class EndpointEngine:
    ...
    def __init__(self, **kwargs: Any) -> None:
        self.model_path = (
            kwargs.get("model_path") or os.environ["ANONLIB_OVERHEAD_MODEL_PATH"]
        )
        self.base_url = os.environ["ANONLIB_OVERHEAD_SGLANG_BASE_URL"].rstrip("/")
        self.timeout_seconds = int(
            os.environ.get("ANONLIB_OVERHEAD_TIMEOUT_SECONDS", "900")
        )
        self.max_workers = max(
            1, int(os.environ.get("ANONLIB_OVERHEAD_CONCURRENCY", "64"))
        )
    ...
    def generate(
        ...
        **_: Any,
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
            futures = [
                pool.submit(self._generate_one, item, params) for item in prompts
            ]
