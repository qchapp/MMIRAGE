...
This experiment measures how much throughput MMIRAGE retains relative to a minimal raw SGLang client when both use the same one-GPU SGLang HTTP server, model, workload, generation settings, and concurrency.
...
2. `mmirage_sglang`: runs the complete MMIRAGE pipeline while forwarding MMIRAGE SGLang calls to the same HTTP endpoint.
...
- `throughput_retention`: mean MMIRAGE throughput divided by mean raw SGLang throughput.
...
| `configs/mmirage_sglang.yaml` | MMIRAGE config used by the MMIRAGE path. |
...
| `scripts/run_mmirage_with_sglang_endpoint.py` | Benchmark-only adapter that forwards MMIRAGE SGLang calls to the HTTP endpoint. |
...
- Python environment with MMIRAGE GPU dependencies and SGLang `0.5.10`.
...
python experiments/raw_sglang_overhead/scripts/run.py \
  ...
  --output-dir /tmp/mmirage_overhead_dry_run \
  --repetitions 1 \
  --dry-run
```

Expected result: the command prints JSON with `"dry_run": true` and writes `/tmp/mmirage_overhead_dry_run/experiment_metadata.json`. Delete or change `/tmp/mmirage_overhead_dry_run` before repeating the smoke check if you want a clean output directory.
...
python experiments/raw_sglang_overhead/scripts/prepare_workload.py \
  --output-dir /tmp/mmirage_overhead_workload \
  --num-rows 1000 \
  --warmup-rows 16 \
  --model-path Qwen/Qwen3-4B
```

If you regenerate, replace `experiments/raw_sglang_overhead/workload` with `/tmp/mmirage_overhead_workload` in the full run command.
...
python experiments/raw_sglang_overhead/scripts/run.py \
  --workload-dir experiments/raw_sglang_overhead/workload \
  --output-dir /tmp/mmirage_overhead_results \
  --repetitions 3 \
  --gpu-count 1 \
  --gpu-index 0 \
  --concurrency 64 \
...
Each repetition starts one fresh SGLang server for `raw_sglang` and one fresh SGLang server for `mmirage_sglang`. With `--repetitions 3`, the model is loaded six times.
...
python experiments/raw_sglang_overhead/scripts/run.py \
  --workload-dir experiments/raw_sglang_overhead/workload \
  --output-dir /tmp/mmirage_overhead_results_h100 \
  --repetitions 3 \
  --gpu-count 1 \
  --gpu-index 0 \
  --concurrency 64 \
...
| `rep_*/mmirage_sglang/` | MMIRAGE outputs, state, stats, and SGLang logs. |

Expected tree after the full command with three repetitions:

```text
/tmp/mmirage_overhead_results/
...
- `throughput retention = mean(MMIRAGE tok/s) / mean(raw SGLang tok/s)`
...
| Raw and MMIRAGE success counts differ | One path failed rows or timed out. | Inspect `raw_results.csv`, `rep_*/raw_sglang/summary.json`, and `rep_*/mmirage_sglang/state/shard_0/status.json`. |
...
- Git commit from `experiment_metadata.json` field `mmirage_commit`.
...
| A100 | MMIRAGE over SGLang | 5026.21 +/- 4.98 | 4.92 +/- 0.01 | 203.00 +/- 0.14 | 1000/1000 |
| H100 | Raw SGLang 0.5.10 | 8781.55 +/- 3.16 | 8.60 +/- 0.00 | 116.28 +/- 0.04 | 1000/1000 |
| H100 | MMIRAGE over SGLang | 8376.01 +/- 3.36 | 8.20 +/- 0.01 | 121.92 +/- 0.05 | 1000/1000 |
